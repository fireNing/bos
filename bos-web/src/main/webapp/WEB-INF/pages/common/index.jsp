...
<script type="text/javascript">
	...
	$(function() {
		...
		// 基本功能菜单加载
		$.ajax({
			url : '${pageContext.request.contextPath}/FunctionAction_findMenuByuserFunction',
			type : 'POST',
			dataType : 'text',
		...
		$("#btnEp").click(function(){
			var isTrue = $("#editPasswordForm").form("validate");
			if(isTrue){
				var password = $("#txtNewPass").val();
				var repassword = $("#txtRePass").val();
				if(password==repassword){
					$.post(
						"${pageContext.request.contextPath}/UserAction_editPassword",
						{password:password},
						function(data){
							if(data == '1'){
								//密码修改成功！
								...
								//密码修改不成功！
								$.messager.alert("提示信息","密码修改失败!请重新修改!","warning");
							}
						}
					);
				}else{
					$.messager.alert("提示信息","两次密码不一致","error");
				}
			}
