...
<script type="text/javascript">
	$(function(){
		...
		$('#save').click(function(){
			//进行表单校验
			var isOk = $("#userForm").form("validate");
			if(isOk){
				$('#userForm').submit();
			}
		});
...
<body class="easyui-layout" style="visibility:hidden;">
    ...
    <div region="center" style="overflow:auto;padding:5px;" border="false">
       <form id="userForm" method="post" action="${pageContext.request.contextPath }/UserAction_addUser">
           <table class="table-edit"  width="95%" align="center">
				...
				<tr>
					<td>联系电话</td>
					<td colspan="3">
						<input type="text" data-options="validType:'checkTele'" name="telephone" id="telephone" class="easyui-validatebox" required="true" />
					</td>
				</tr>
	           	...
	           	<tr><td>选择角色:</td>
	           		<td colspan="3" id="roleTD">
	           			<script type="text/javascript">
	           				$(function(){
	           					$.post(
	           						"${pageContext.request.contextPath}/RoleAction_findAllRole",
	           						function(data){
	           							//将角色数据放到选择角色中
	           							for(var i = 0 ; i < data.length; i++){
	           								$("#roleTD").append("<input name='roleIds' type='checkbox' id='"+data[i].id+"' value='"+data[i].id+"'/><label for='"+data[i].id+"'>"+data[i].name+"</label>");
	           							}
	           						},
	           						"json"
	           					);
