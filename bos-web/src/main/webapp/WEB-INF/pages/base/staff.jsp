...
<script type="text/javascript">
	...
	function doDelete(){
		var deleteList = $("#grid").datagrid("getSelections");
		if(deleteList.length == 0){
			$.messager.alert("提示信息","请选中删除的取派员！");
		}else{
			$.messager.confirm("提示信息","确定删除取派员吗？",function(y){
				if(y){
					var array = new Array();
					for(var i = 0 ; i < deleteList.length;i++){
						var staff = deleteList[i];
						var id = staff.id;
						array.push(id);
					}
					var ids = array.join(",");
	...
	$(function(){
		...
		$('#grid').datagrid( {
			...
			pagination : true,
			toolbar : toolbar,
			url : "${pageContext.request.contextPath}/StaffAction_staffList",
			idField : 'id',
			columns : columns,
	...
	$(function(){
		$("#save").click(function(){
			var checkSuccess = $("#addStaff").form("validate");
			if(checkSuccess){
				$("#addStaff").submit();
			}
		});
	...
	$(function(){
		$("#edit").click(function(){
			var isOk = $("#editStaff").form("validate");
			if(isOk){
				$("#editStaff").submit();
			}
		});
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="对收派员进行添加或者修改" id="addStaffWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div region="center" style="overflow:auto;padding:5px;" border="false">
			<form id="addStaff" action="${pageContext.request.contextPath }/StaffAction_addStaff" method="post">
				<table class="table-edit" width="80%" align="center">
					...
					<tr>
						<td>手机</td>
						<td><input data-options="validType:'checkTele'" type="text" name="telephone" class="easyui-validatebox" required="true"/></td>
					</tr>
					<tr>
						<td>单位</td>
